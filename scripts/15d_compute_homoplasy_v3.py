"""
Robust seed-based homoplasy v3: 
- For '-' strand genes, search revcomp seed (not forward seed)
- For '+' strand genes, search forward seed
- Always use best-match verification
"""
...
def find_forward_seed(assembly_seq, seed, gene_len):
    """Find all forward-strand occurrences of seed. Returns [(pos, matches_in_verify), ...]"""
    results = []
    verify_len = min(VERIFY_LEN, gene_len)
    start = 0
    while True:
        ...
        if end_pos <= len(assembly_seq):
            # Count matches vs what we expect (the seed was the start, so verify matches at offset)
            extracted = assembly_seq[pos:end_pos]
            # matches is just a sanity check - count of identical bases in verify window
            # We'll just note the position; actual verification is done later
            results.append(('fwd', pos))
        start = pos + 1
    return results


def find_revcomp_seed(assembly_seq, seed, gene_len):
    """Find all revcomp-strand occurrences of seed (seed is coding seq, revcomp is forward strand)."""
    results = []
    ...
    while True:
        ...
        if pos == -1:
            break
        results.append(('rev', pos))
        start = pos + 1
    return results


def verify_match(assembly_seq, pos, strand, ref_seq):
    """Extract gene at position and verify against reference. Returns (gene_seq, match_score) or None.
    For '-' strand: extracted forward strand is revcomp'd before comparison (ref_seq is coding seq)."""
    gene_len = len(ref_seq)
    end_pos = pos + gene_len
    if end_pos > len(assembly_seq):
        return None, -1
    
    extracted = assembly_seq[pos:end_pos]
    
    # For '-' strand genes, the extracted forward strand must be reverse-complemented
    # to match the coding-sequence reference
    if strand == '-':
        extracted_coding = str(Seq(extracted).reverse_complement())
    else:
        extracted_coding = extracted
    
    matches = sum(1 for a, b in zip(extracted_coding, ref_seq) if a == b)
    score = matches / gene_len
    return extracted_coding, score
...
def main():
    ...
    for gi, gf in enumerate(genome_files):
        ...
        for gene, (start, end, strand) in RESISTANCE_GENES.items():
            ...
            # Find candidates based on strand
            candidates = []
            
            if strand == '+':
                candidates = find_forward_seed(assembly_seq, seed, gene_len)
            else:
                # '-' strand: seed is coding seq start. Find revcomp in forward assembly.
                candidates = find_revcomp_seed(assembly_seq, seed, gene_len)
            
            if not candidates:
                continue
            
            # Verify each candidate, pick best
            best_seq = None
            best_score = 0.0
            
            for s_type, pos in candidates:
                extracted, score = verify_match(assembly_seq, pos, s_type, ref_seq)
                if score > best_score:
                    best_score = score
                    best_seq = extracted
            
            # Require >= 95% identity
            if best_score < 0.95 or best_seq is None:
                continue
            ...
            asm_prot = str(Seq(best_seq).translate())
        ...
        if (gi + 1) % 10 == 0:
            print(f"  Processed {gi+1}/{len(genome_files)} genomes ({genes_found}/{len(RESISTANCE_GENES)} genes found in current)")
